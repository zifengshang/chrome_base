...
bool JSONWriter::BuildJSONString(const Value& node, size_t depth) {
  switch (node.GetType()) {
    ...
    case Value::Type::BINARY:
      ...
      // yinzhou: encode binary value to string to set into json string.
      // modified by bingoli 2017.8.9
      if (!node.GetBuffer()) {
        NOTREACHED() << "binary data pointer is nullptr.";
      }
      else {
        StringPiece data;
        data.set(node.GetBuffer(), node.GetSize());
        std::string encoded_data;
        base::Base64Encode(data, &encoded_data);
        std::string value;
        size_t reserved_size = encoded_data.size() +
          strlen(BINARY_BASE64_ENCODE_HEADER) +
          strlen(BINARY_BASE64_ENCODE_TAIL);
        value.reserve(reserved_size);
        value += BINARY_BASE64_ENCODE_HEADER;
        value += encoded_data;
        value += BINARY_BASE64_ENCODE_TAIL;
        EscapeJSONString(value, true, json_string_);
        return true;
      }
      break;
