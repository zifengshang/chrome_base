...
#ifndef NOT_QB_BASE_BUILD
  // yinzhou: may be binary value
  int header_length = strlen(BINARY_BASE64_ENCODE_HEADER);
  int tail_length = strlen(BINARY_BASE64_ENCODE_TAIL);
  StringPiece string_piece = string.AsStringPiece();
  if ((int)string_piece.size() > (header_length + tail_length)) {
    if (0 == strncmp(string_piece.data(), BINARY_BASE64_ENCODE_HEADER,
      header_length)) {
      const char* str = string_piece.data() + string_piece.size() - tail_length;
      if (0 == strncmp(str, BINARY_BASE64_ENCODE_TAIL, tail_length)) {
        string_piece.remove_prefix(header_length);
        string_piece.remove_suffix(tail_length);
        std::string output;
        if (base::Base64Decode(string_piece, &output))
          return base::Value::CreateWithCopiedBuffer(
            output.c_str(), output.size());
        else {
          DLOG(WARNING) << "Decode binary data failed.";
        }
      }
    }
  }
